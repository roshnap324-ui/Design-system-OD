...
## Source of truth
`ooredoo_design_system_complete-2.html` is the canonical design system. Its `:root`
...
1. **Strictly follow the existing design system properties.** Use the DS tokens and
   components defined in `ooredoo_design_system_complete-2.html`. Reuse existing
...
3. **Adding new values is allowed only when missing** from the DS. If a Figma color/
   ...
   - Add it to `:root` in `ooredoo_design_system_complete-2.html` using the **proper
